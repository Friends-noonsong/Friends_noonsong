...
public class GPSManager : MonoBehaviour
{
    ...
    public TalkDialog talkDialog; // TalkDialog 클래스 인스턴스, 다이얼로그 호출에 사용
    ...
    private void TriggerDialog(int index)
    {
        ...
        {
            case 0:
                talkDialog.FirstDialog();
                break;
            case 1:
                if (talkDialog.IsDialogTriggered(0)) // 이전 다이얼로그가 호출되었는지 확인
                    talkDialog.SecondDialog();
                break;
            case 2:
                if (talkDialog.IsDialogTriggered(1)) // 이전 다이얼로그가 호출되었는지 확인
                    talkDialog.ThirdDialog();
                break;
            case 3:
                if (talkDialog.IsDialogTriggered(2)) // 이전 다이얼로그가 호출되었는지 확인
                    talkDialog.ForthDialog();
                break;
        }
    }
}