...
public class TalkDialogue : MonoBehaviour // TalkDialogue는 튜토리얼 전체 대사가 들어있음
{
    ...
    public Animator snowflakeAnimator; // snowflake 오브젝트의 Animator
    ...
    private void FirstDialog()
    {
        ...
        FirstDialog.Add(new DialogData("/color:black/[학교가 너무 처음이라 막막하네...]", "User", () => StartCoroutine(NoonDungMove()))); // 3d 오브젝트 화면상에서 움직이는 코드
        ...
        FirstDialog.Add(new DialogData("/color:black/어디? 어디?", "NoonDung", () => StartCoroutine(SnowflakeMove())));
        ...
        FirstDialog.Add(new DialogData("/color:black/마침 우리한테 꿍쳐놓은 눈의 결정이 있으니까, 너한테 줄게!", "Snowflake", () => ChangeAnimation(snowflakeAnimator, "standing")));
        ...
        // part3 로로 등장
        FirstDialog.Add(new DialogData("/color:black//wait:1/앗! 찾았다", "RoRo", () => StartCoroutine(RoroMove())));
    ...
    private IEnumerator NoonDungMove()
    {
        // 시작 위치와 끝 위치 설정
        ...
        // 눈덩이 초기 위치 설정
        noonDung.transform.position = startPos;
        ...
        while (elapsedTime < moveDuration)
        {
            ...
            noonDung.transform.position = Vector3.Lerp(startPos, endPos, t);
            yield return null;
        }
    }

    private IEnumerator RoroMove()
    {
        // 시작 위치와 끝 위치 설정
        Vector3 startPos = arCamera.TransformPoint(new Vector3(0f, 0f, 10f)); // AR 카메라의 뒤에서 달려옴
        Vector3 endPos = arCamera.TransformPoint(new Vector3(0f, 0f, 2f)); // AR 카메라의 정면 (화면 중앙)

        // 눈덩이 초기 위치 설정
        roro.transform.position = startPos;
        ...
        while (elapsedTime < moveDuration)
        {
            elapsedTime += Time.deltaTime;
            float t = Mathf.Clamp01(elapsedTime / moveDuration);
            roro.transform.position = Vector3.Lerp(startPos, endPos, t);
            yield return null;
        }
    }

    private IEnumerator SnowflakeMove()
    {
        // 시작 위치와 끝 위치 설정
        Vector3 startPos = arCamera.TransformPoint(new Vector3(0f, 5f, 0f)); // AR 카메라의 위에서 내려옴
        Vector3 endPos = arCamera.TransformPoint(new Vector3(0f, 0f, 1f)); // AR 카메라의 정면 (화면 중앙)

        // 눈덩이 초기 위치 설정
        snowflake.transform.position = startPos;
        ...
        while (elapsedTime < moveDuration)
        {
            elapsedTime += Time.deltaTime;
            float t = Mathf.Clamp01(elapsedTime / moveDuration);
            snowflake.transform.position = Vector3.Lerp(startPos, endPos, t);
            yield return null;
        }
    }
}