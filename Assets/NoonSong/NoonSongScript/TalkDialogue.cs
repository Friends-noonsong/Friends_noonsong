...
public class TalkDialogue : MonoBehaviour // TalkDialogue는 튜토리얼 전체 대사가 들어있음
{
    ...
    public void FirstDialog()
    {
        ...
        FirstDialog.Add(new DialogData("/color:black/[학교가 너무 처음이라 막막하네...]", "User", () => StartCoroutine(MoveObject(noonDung, arCamera.TransformPoint(new Vector3(-1.5f, 1.5f, 5f)), arCamera.TransformPoint(new Vector3(0f, -0.4f, 1.5f)))))); // 좌측 위 등장 눈덩이 시작)))));
        ...
        // part3 로로 등장
        FirstDialog.Add(new DialogData("/color:black//wait:1/앗! 찾았다", "RoRo", () => StartCoroutine(MoveObject(roro, arCamera.TransformPoint(new Vector3(0f, 1f, 5f)), arCamera.TransformPoint(new Vector3(0f, -0.2f, 1f)))))); // 멀리서 달려오듯이 등장 로로 시작
        ...
        // part4 눈결이 등장
        FirstDialog.Add(new DialogData("/color:black/[저기 하늘에 떠 다니는 건 뭐지?]", "User", () => StartCoroutine(MoveObject(noonkyeol, arCamera.TransformPoint(new Vector3(1f, -0.5f, 5f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 3f)))))); // 오른쪽에서 천천히 등장 눈결이 시작));
        FirstDialog.Add(new DialogData("/color:black/저기,, 안녕하세요! 처음 보는 분이네요..!", "NoonGyeol"));
        FirstDialog.Add(new DialogData("/color:black/눈결이 안녕! 혹시 근처에서 눈송이 못봤어?", "NoonDung"));
        ...
        FirstDialog.Add(new DialogData("/color:black/앗, 그렇다면 이게 도움이 될 거예요!", "NoonDung"));
        FirstDialog.Add(new DialogData("/color:black/저는 지도를 잘 보거든요. 그 외에 이것저것 많은 것을 알고 있으니까, 제 지식이 도움이 될 수 있을 것 같아요.", "NoonGyeol"));
        ...
        // part5 꽃송이 등장
        FirstDialog.Add(new DialogData("/color:black/안녕, 친구들? 처음 보는 친구도 있구나! 새송이인가 보네?", "Kkotsong", () => StartCoroutine(MoveObject(kkotsong, arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)))))); // 화면 가운데서 춤 연습중 꽃송이 시작
        FirstDialog.Add(new DialogData("/color:black/이 친구는 꽃송이야! 눈송이의 베프인 꽃송이라면 눈송이가 어디 있는지 알 지도 몰라!", "RoRo"));
        ...
        FirstDialog.Add(new DialogData("/color:black/그렇다면 정확히 찾아 왔어. 마침 방금 전까지 눈소이랑 함께 있던 참이었거든.", "Kkotsong"));
        ...
        // part6,7
        FirstDialog.Add(new DialogData("/color:black/찾았다, 눈송이!", "Kkotsong", () => StartCoroutine(MoveObject(noonsong, arCamera.TransformPoint(new Vector3(-1f, -0.5f, 2f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)))))); // 화면 좌측에서 등장 눈송이 시작
        FirstDialog.Add(new DialogData("/color:black/안녕, 친구들! 어라, 처음 보는 친구도 있네?", "NoonSong"));
        FirstDialog.Add(new DialogData("/color:black/이 애가 너와 친구가 되고 싶다고 해서 데려왔어!", "NoonDung"));
        FirstDialog.Add(new DialogData("/color:black/눈송이를 위한 선물도 가져왔어! (두근두근…!)", "Snowflake"));
        FirstDialog.Add(new DialogData("/color:black/와아, 눈의 결정이네! 정말 기뻐!", "NoonSong"));
        FirstDialog.Add(new DialogData("/color:black/이렇게 찾아와 줘서 고마워, 그럼 우리 오늘부터 친구 하자!", "NoonSong"));
        FirstDialog.Add(new DialogData("/color:black/[(눈송이와 친구가 되자.)]", "User", () => noonsong.SetActive(false)));
        ...
        FirstDialog.Add(new DialogData("/color:black/[(인사한다.)]", "Turi"));
        ...
        FirstDialog.Add(new DialogData("/color:black/[(고개를 끄덕인다.)]", "User"));
        FirstDialog.Add(new DialogData("/color:black/흐흥, 미래 산업시대의 선구자가 될 이 튜리님에게 너 같은 친구들을 위한 발명품이 있지!", "Turi"));
        FirstDialog.Add(new DialogData("/color:black/바로 /color:blue/특별한 눈의 결정/color:black/! 특별한 눈송이들의 마음도 사로잡을 수 있는 대단한 아이템이야!", "Turi"));
        ...
        FirstDialog.Add(new DialogData("/color:black/튜토리얼을 성공적으로 마친 당신에게 이것을 드립니다!"));
        FirstDialog.Add(new DialogData("/color:black/다양한 눈송이들과 친구가 되어 완벽한 학생증을 완성하는 것을 목표로 힘을 내 봐요!"));
        FirstDialog.Add(new DialogData("/color:black/도감을 다 채우고 학생증을 완성한다면, 특별한 선물이 있을 지도…!?"));
    ...
    public void SecondDialog()
    {
        ...
        // part4 눈결이 등장
        SecondDialog.Add(new DialogData("/color:black/[저기 하늘에 떠 다니는 건 뭐지?]", "User", () => StartCoroutine(MoveObject(noonkyeol, arCamera.TransformPoint(new Vector3(1f, -0.5f, 5f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 3f)))))); // 오른쪽에서 천천히 등장 눈결이 시작));
        SecondDialog.Add(new DialogData("/color:black/저기,, 안녕하세요! 처음 보는 분이네요..!", "NoonGyeol"));
        SecondDialog.Add(new DialogData("/color:black/눈결이 안녕! 혹시 근처에서 눈송이 못봤어?", "NoonDung"));
        ...
        SecondDialog.Add(new DialogData("/color:black/앗, 그렇다면 이게 도움이 될 거예요!", "NoonDung"));
        SecondDialog.Add(new DialogData("/color:black/저는 지도를 잘 보거든요. 그 외에 이것저것 많은 것을 알고 있으니까, 제 지식이 도움이 될 수 있을 것 같아요.", "NoonGyeol"));
        ...
        SecondDialog.Add(new DialogData("/color:black/2캠퍼스 정문으로 가보자!", "RoRo", () => noonkyeol.SetActive(false)));
    ...
    public void ThirdDialog()
    {
        ...
        // part5 꽃송이 등장
        ThirdDialog.Add(new DialogData("/color:black/안녕, 친구들? 처음 보는 친구도 있구나! 새송이인가 보네?", "Kkotsong", () => StartCoroutine(MoveObject(kkotsong, arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)))))); // 화면 가운데서 춤 연습중 꽃송이 시작
        ThirdDialog.Add(new DialogData("/color:black/이 친구는 꽃송이야! 눈송이의 베프인 꽃송이라면 눈송이가 어디 있는지 알 지도 몰라!", "RoRo"));
        ...
        ThirdDialog.Add(new DialogData("/color:black/그렇다면 정확히 찾아 왔어. 마침 방금 전까지 눈소이랑 함께 있던 참이었거든.", "Kkotsong"));
        ThirdDialog.Add(new DialogData("/color:black/아마 눈송이는 /color:blue/프라임관/color:black/에 있을 거야! 어딘지 아니? 같이 가 줄게.", "Kkotsong", () => kkotsong.SetActive(false)));
    ...
    public void ForthDialog()
    {
        ...
        ForthDialog.Add(new DialogData("/color:black/찾았다, 눈송이!", "Kkotsong", () => StartCoroutine(MoveObject(noonsong, arCamera.TransformPoint(new Vector3(-1f, -0.5f, 2f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)))))); // 화면 좌측에서 등장 눈송이 시작
        ForthDialog.Add(new DialogData("/color:black/안녕, 친구들! 어라, 처음 보는 친구도 있네?", "NoonSong"));
        ForthDialog.Add(new DialogData("/color:black/이 애가 너와 친구가 되고 싶다고 해서 데려왔어!", "NoonDung"));
        ForthDialog.Add(new DialogData("/color:black/눈송이를 위한 선물도 가져왔어! (두근두근…!)", "Snowflake"));
        ForthDialog.Add(new DialogData("/color:black/와아, 눈의 결정이네! 정말 기뻐!", "NoonSong"));
        ForthDialog.Add(new DialogData("/color:black/이렇게 찾아와 줘서 고마워, 그럼 우리 오늘부터 친구 하자!", "NoonSong"));
        ForthDialog.Add(new DialogData("/color:black/[(눈송이와 친구가 되자.)]", "User", () => noonsong.SetActive(false)));
        ForthDialog.Add(new DialogData("/color:black/앗-! 다들 나만 빼고 여기 모여 있었구나!", "Turi", () => StartCoroutine(MoveObject(turi, arCamera.TransformPoint(new Vector3(0f, -2f, 2f)), arCamera.TransformPoint(new Vector3(0f, 0f, 2f)))))); // 아래에서 등장 튜리 시작
        ForthDialog.Add(new DialogData("/color:black/어라? 못 보던 얼굴도 있네?", "Turi"));
        ForthDialog.Add(new DialogData("/color:black/[(인사한다.)]", "Turi"));
        ForthDialog.Add(new DialogData("/color:black/안녕, 튜리! 이 애는 새로운 눈송이인데, 나랑 친구가 되고 싶다고 찾아와 줬어!", "NoonSong"));
        ForthDialog.Add(new DialogData("/color:black/오… 이해했어!", "Turi"));
        ForthDialog.Add(new DialogData("/color:black/거기 눈송, 좀 더 다양한 눈송이를 만나보고 싶지 않아?", "Turi"));
        ForthDialog.Add(new DialogData("/color:black/[(고개를 끄덕인다.)]", "User"));
        ForthDialog.Add(new DialogData("/color:black/흐흥, 미래 산업시대의 선구자가 될 이 튜리님에게 너 같은 친구들을 위한 발명품이 있지!", "Turi"));
        ForthDialog.Add(new DialogData("/color:black/바로 /color:blue/특별한 눈의 결정/color:black/! 특별한 눈송이들의 마음도 사로잡을 수 있는 대단한 아이템이야!", "Turi"));
        ...
        ForthDialog.Add(new DialogData("/color:black/튜토리얼을 성공적으로 마친 당신에게 이것을 드립니다!"));
        ForthDialog.Add(new DialogData("/color:black/다양한 눈송이들과 친구가 되어 완벽한 학생증을 완성하는 것을 목표로 힘을 내 봐요!"));
        ForthDialog.Add(new DialogData("/color:black/도감을 다 채우고 학생증을 완성한다면, 특별한 선물이 있을 지도…!?"));
    ...
    private void Start()
    {
        ...
        //StartCoroutine(MoveObject(snowflake, arCamera.TransformPoint(new Vector3(0.35f, 2f, -1f)), arCamera.TransformPoint(new Vector3(0.35f, 0f, -1f)))); // 하늘에서 내려옴 눈꽃송이 시작
        //StartCoroutine(MoveObject(roro, arCamera.TransformPoint(new Vector3(0f, 1f, 5f)), arCamera.TransformPoint(new Vector3(0f, -0.2f, 1f)))); // 멀리서 달려오듯이 등장 로로 시작
        //StartCoroutine(MoveObject(noonkyeol, arCamera.TransformPoint(new Vector3(1f, -0.5f, 5f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 3f)))); // 오른쪽에서 천천히 등장 눈결이 시작
        //StartCoroutine(MoveObject(kkotsong, arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)), arCamera.TransformPoint(new Vector3(0f, -0.5f, 2f)))); // 화면 가운데서 춤 연습중 꽃송이 시작
